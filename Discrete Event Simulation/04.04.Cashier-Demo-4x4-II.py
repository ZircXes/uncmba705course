"""
Example 04.04
Model for 4x4 Line and Cashier for Grocery Store
This model selects the shortest line when assigning a station.
...
MODEL_NAME = "Grocery Store Demo: 4 Lines and 4 Cashiers - Part II"

# Pick shortest StopIteration (CHANGE A04)
def get_station_with_shortest_queue(station_list):
  best_station = station_list[0]
  current_queue = len(station_list[0].queue)

  for i in station_list:
    if len(i.queue) < current_queue:
      best_station = i
      current_queue = len(i.queue)
...
def customer_arrivals(env):
  ...
  while True:
    ...
    station = get_station_with_shortest_queue(grocery_stations) # CHANGE A04
